...
public class AptController_companymodifyagain {
...
//------------------------------------------------------------------------------((진경)) 	
	...
	public void updateMyinfo() {
		...
		System.out.println("▶ 아이디를 입력해주세요 ");
		String id = sc.next();
		MemberVO member = memDAO.getMember(id); // 멤버만 신청 가능하니까 멤버아이디만 유효 검사
		ApplicationVO app = appDAO.getAppMember(id);
	...
	public void remove() {
		...
		if (answer.equals("y") && pw.equals(member.getMemPw())) {
			dao.remove(id);
...
//------------------------------------------------------------------------------((순근)) 	
	...
	public void createAptInfo() {
		...
		int AptId = dao.countaptid() + 1; //dao의 countaptid메서드
	...
	public void updateAptInfo() {
		...
		CompanyVO cvo = cao.getAptId(findAptNum);
		...
		if (findAptNum == avo.getAptId()) {
			...
			cvo = new CompanyVO(findAptNum, ComName, AptName);
			dao.updateAptInfo(avo);
			cao.updateAptInfo(cvo);
